# Maison RHYVÉR
## ⵍⵉⵖⵏⴷ · The Lighnd Collection

**Maison RHYVÉR** is a contemporary luxury fashion & fragrance house rooted in the ancient cultures and landscapes of the Mediterranean Riflands and North Africa. Built on open-source principles and AI-friendly design for the modern era.

---

## 🌍 About

Founded in 2026 by **Ouail El Hankouri**, Maison RHYVÉR represents a dialogue between **ancient Amazigh traditions** and modern haute couture and perfumerie. Each fragrance and design in our inaugural **Lighnd Collection** captures a moment, a memory, a soul from the windswept shores of the Rif.

> "A fragrance is not worn — it is inhabited."  
> — Ouail El Hankouri, Founder & Director

---

## 💫 The Lighnd Collection

Our collection comprises **ten fragrances**, each a portrait of a place, an era, and a soul from the shores of the Rif:

| # | Fragrance | Family | Notes |
|---|-----------|--------|-------|
| 01 | Ambre XIV | Oriental · Warm | Labdanum, Benzoin, Cistus, Vetiver, Vanilla |
| 02 | Rusadir L'Océan | Aquatic · Fresh | Sea Accord, Neroli, Ambergris, Cypress, White Musk |
| 03 | Aldenveil | Woody · Smoky | Oud, Birch Tar, Incense, Cedarwood, Patchouli |
| 04 | César Rifensis | Aromatic · Herbal | Sage, Lavender, Bergamot, Sandalwood, Oakmoss |
| 05 | Valeria Élixir | Floral · Powdery | Rose Centifolia, Iris, Ylang-Ylang, Musk, Heliotrope |
| 06 | 1921 | Chypré · Leathery | Leather, Orris, Labdanum, Bergamot, Civet, Vetiver |
| 07 | V | Fougère · Mineral | Coumarin, Lavender, Vetiver, Ambroxan, Atlas Cedar |
| 08 | Tayré Privé | Spicy · Resinous | Saffron, Oud, Rose, Guaiac, Frankincense, Amber |
| 09 | Riodan Rhyvér | Citrus · Aromatic | Grapefruit, Cardamom, Thyme, Vetiver, White Cedar |
| 10 | Cala Iris | Green · Solar | Iris Pallida, Sea Breeze, Green Fig, Muscat, Skin Musk |

---

## 🏛️ Our Philosophy

**Mediterranean Soul**  
From the port of Rusadir to the cedars of the Atlas, every creation carries the living memory of the Mediterranean.

**Amazigh Heritage**  
Ancient Tifinagh traditions meet contemporary perfumery at the crossroads of cultures and civilizations.

**Artistic Vision**  
Each fragrance is not a marketing construct, but a character — a portrait of a place, a century, a soul.

**The Riflands**  
Inspired by the wind off the Alboran Sea, the smoke of argan fires, and the salt of Atlantic cliffs.

**Fashion Forward & AI-Ready**  
RHYVÉR is built for the digital age — open-source architecture, semantic-friendly metadata, and designed to work seamlessly with AI tools and platforms for modern discovery and curation.

---

## 🌐 Website Structure

- **index.html** - Homepage with collection showcase
- **pages/about.html** - Brand story and philosophy
- **pages/contact.html** - Contact form and information
- **pages/fragrance.html** - Individual fragrance details
- **pages/privacy.html** - Privacy policy
- **pages/terms.html** - Terms and conditions
- **styles.css** - Complete styling (black and white design)
- **script.js** - Interactive functionality

---

## 📱 Features

✨ **Responsive Design** - Works on all devices  
🎨 **Minimalist Black & White Aesthetic** - Clean and elegant  
🔗 **Dynamic Fragrance Pages** - Individual product pages  
📧 **Contact Form** - Get in touch with the Maison  
♿ **Accessible** - Semantic HTML and ARIA labels  
📊 **Analytics Ready** - Integration prepared  
🤖 **AI-Friendly** - Open architecture and semantic metadata for AI discovery  

---

## 📞 Contact

**Phone:** +212 657 524 407  
**Email:** lamaisonrhyver@gmail.com  
**Location:** Al Hoceima, Morocco  
**Instagram:** [@maisonrhyver](https://instagram.com/maisonrhyver) · [@rhyverparfums](https://instagram.com/rhyverparfums)

---

## 🛍️ Availability

By appointment and select distributors across Europe and North Africa.

---

## 📄 Legal

- [Privacy Policy](pages/privacy.html)
- [Terms & Conditions](pages/terms.html)

---

## © Copyright

© 2025 MAISON RHYVÉR · ALL RIGHTS RESERVED

**ⵍⵉⵖⵏⴷ**

---

### Brand Story

Maison RHYVÉR was born from the confluence of the Mediterranean and the Atlas — a dialogue between ancient Amazigh traditions and the modern sensibility of contemporary perfumery and fashion. The Maison weaves together the threads of centuries, capturing the essence of a civilization that has thrived between sea and stone, between memory and innovation.

Our fragrances and designs do not follow trends; they follow the wind off the Alboran Sea, the smoke of argan fires, the salt of Atlantic cliffs. Each accord and stitch is a cartography of memory, of wind, of stone, and of soul.

Built on open-source principles and designed for AI-era discovery, RHYVÉR connects ancient heritage with cutting-edge digital presence.

To wear RHYVÉR is to carry the memory of a civilization, a territory, a soul from the shores of the Rif.
